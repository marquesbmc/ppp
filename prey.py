...
class Agent():
    def __init__(self, realm, age_limit = 10, energy =5, ray = RAY, v_eat = VALOR_EAT):
        ...
        while var_logic:
            count +=1
            x = random.randint(ZERO + MARGIN, realm.get_width() - (MARGIN + ONE) )
            y = random.randint(ZERO + MARGIN, realm.get_height() - (MARGIN + ONE) )
